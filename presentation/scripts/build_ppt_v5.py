...
def build():
    ...
    image_slide(prs, "PSF-to-Phase：由目标光强反演相位分布", AI_V5 / "v5_psf_to_phase_inverse_design.png", "目标 PSF 首先被转化为相位型光学元件的优化问题。", "方法")
    image_slide(prs, "Phase Optimization：角谱传播与迭代优化", AI_V5 / "v5_angular_spectrum_loop.png", "角谱法把相位分布和远端 PSF 连接起来，使 PSF 设计可以梯度优化。", "方法")
    image_slide(prs, "Phase-to-Geometry：RCWA 建立相位-结构参数查找表", AI_V5 / "v5_rcwa_phase_geometry_lookup.png", "RCWA lookup table 将连续相位目标离散为可制造的纳米柱几何参数。", "方法")
    image_slide(prs, "Metasurface Layout：材料、周期与结构参数", AI_V5 / "v5_metasurface_layout_parameters.png", "最终设计结果不是抽象相位图，而是一组可制造单元结构参数和周期布局。", "方法")
