...
PPTX_PATH = OUT_DIR / "metasurface_industrial_defect_detection_v3.pptx"
...
def build():
    ...
    cover(prs)
    image_slide(prs, "背景：工业检测计算瓶颈与超表面视觉前端机会", AI_V2 / "v2_background_infographic.png", "工业质检需要高速低功耗视觉前端，超表面提供了把早期卷积前移到成像链路的入口。", "背景")
    image_slide(prs, "总体方法：传统电子模型到光电融合 student", AI_V2 / "v2_methodology_graphical_abstract.png", "主线是 teacher 蒸馏、optical student、kernel-to-PSF 和轻量电子后端。", "方法")
    image_slide(prs, "光电融合系统：metasurface + CMOS + electronic backend", AI_V2 / "v2_hybrid_optical_architecture.png", "超表面承担早期光学卷积，电子后端负责 calibration、非线性和任务输出。", "方法")
    image_slide(prs, "知识蒸馏机制：对齐 score、mask 与 feature volume", AI_V3 / "v3_distillation_training_mechanism.png", "蒸馏把强电子 teacher 的任务能力迁移到受物理约束的 optical student。", "方法", formula_path=kd_formula)
    image_slide(prs, "Kernel-to-PSF：从 signed kernel 到超表面可实现目标", AI_V3 / "v3_kernel_to_psf_mapping.png", "正负拆分是把含符号卷积核映射到非负光强 PSF 的关键步骤。", "方法", formula_path=split_formula)
    image_slide(prs, "两个案例：Fabric 二分类与 DAGM 像素级分割", AI_V3 / "v3_case_overview_two_examples.png", "Fabric 是方法起点，DAGM 是高性能分割和物理映射闭环。", "方法")

    image_slide(prs, "Fabric / AITEX：织物 patch 缺陷二分类任务", AI_V2 / "v2_fabric_case_diagram.png", "Fabric 检测织物 patch 是否存在缺陷，不输出具体缺陷位置。", "Fabric")
    image_slide(prs, "Fabric teacher/student 架构", AI_V3 / "v3_fabric_teacher_student_architecture.png", "R1 student 使用 64×64 输入和一层 optical convolution bank 学习二分类判别。", "Fabric")
    ...
    image_slide(prs, "DAGM Class7：工业纹理缺陷分类 + 像素级分割", AI_V2 / "v2_dagm_case_diagram.png", "DAGM 不只判断有没有缺陷，还输出 pixel-level defect mask，是当前主结果。", "DAGM")
    image_slide(prs, "DAGM teacher/student 架构", AI_V3 / "v3_dagm_teacher_student_architecture.png", "Student 保留 SegDecNet-style 电子后端，把早期 volume 前端替换为 optical convolution bank。", "DAGM")
    image_slide(prs, "DAGM 两阶段蒸馏训练", AI_V3 / "v3_two_stage_dagm_training.png", "先强化 volume/segmentation 对齐，再联合优化分类、分割和任务损失。", "DAGM", formula_path=kd_formula)
    ...
    image_slide(prs, "项目闭环总结", AI_V3 / "v3_project_summary_closed_loop.png", "本课题已经形成 teacher -> optical student -> kernels -> PSF -> metasurface probe -> compute reduction 的证据链。", "总结")
    image_slide(prs, "两个案例对照总结", comparison_table, "Fabric 验证二分类方法起点，DAGM 验证高性能分割和物理映射闭环。", "总结")
    image_slide(prs, "未来展望：从分割到定位检测", AI_V2 / "v2_future_system_roadmap.png", "下一步可扩展到 YOLO、PCB、wafer、chip defect localization，并加入 hardware-aware retraining。", "展望")
