class Config:
    ...
    def init_extra(self):
        ...
        self.TRAIN_MODE = self.TRAIN_MODE.upper()
        if self.TRAIN_MODE not in ["JOINT", "SEG_ONLY"]:
        ...
        if self.INPUT_SIZE is not None:
            if self.DATASET != 'DAGM':
                raise Exception("INPUT_SIZE override is currently intended for square DAGM experiments only.")
            if self.INPUT_SIZE % 8 != 0:
                raise Exception(f"INPUT_SIZE must be divisible by 8, got {self.INPUT_SIZE}")
            self.INPUT_WIDTH = self.INPUT_SIZE
            self.INPUT_HEIGHT = self.INPUT_SIZE
    ...
    def merge_from_args(self, args):
        ...
        if args.TRAIN_MODE is not None: self.TRAIN_MODE = args.TRAIN_MODE
        if args.MAX_TRAIN_SAMPLES is not None: self.MAX_TRAIN_SAMPLES = args.MAX_TRAIN_SAMPLES
        if args.MAX_VAL_SAMPLES is not None: self.MAX_VAL_SAMPLES = args.MAX_VAL_SAMPLES
        if getattr(args, "INPUT_SIZE", None) is not None: self.INPUT_SIZE = args.INPUT_SIZE
...
def load_from_dict(dictionary):
    ...
    cfg.TRAIN_MODE = dictionary.get("TRAIN_MODE", "JOINT")
    cfg.MAX_TRAIN_SAMPLES = dictionary.get("MAX_TRAIN_SAMPLES", None)
    cfg.MAX_VAL_SAMPLES = dictionary.get("MAX_VAL_SAMPLES", None)
    cfg.INPUT_SIZE = dictionary.get("INPUT_SIZE", None)
