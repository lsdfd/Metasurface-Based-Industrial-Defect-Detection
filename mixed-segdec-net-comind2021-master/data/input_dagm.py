...
def limit_balanced_samples(samples, max_samples, class_path, sub_dir):
    ...
    for sample in samples:
        ...
        seg_mask_path = os.path.join(class_path, sub_dir, "Label", f"{img_name_short}_label.PNG")
    ...
    n_pos = min(half, len(pos_samples))
    n_neg = min(max_samples - n_pos, len(neg_samples))
    return pos_samples[:n_pos] + neg_samples[:n_neg]
...
class DagmDataset(Dataset):
    ...
    def read_contents(self):
        ...
        max_samples = self.cfg.MAX_TRAIN_SAMPLES if self.kind == "TRAIN" else self.cfg.MAX_VAL_SAMPLES

        sub_dir = self.kind.lower().capitalize()
        samples = limit_balanced_samples(samples, max_samples, self.path, sub_dir)
