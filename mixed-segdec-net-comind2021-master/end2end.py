...
class End2End:
    ...
    def training_iteration(self, data, device, model, criterion_seg, criterion_dec, optimizer, weight_loss_seg, weight_loss_dec,
        ...
        for sub_iter in range(num_subiters):
            ...
            if is_segmented[sub_iter]:
                ...
                if self.cfg.TRAIN_MODE == "SEG_ONLY":
                    loss_dec = torch.zeros((), device=device)
                else:
                ...
                score = self._get_sample_score(decision, output_seg_mask)
                total_correct += (score > 0.0).item() == is_pos_.item()
    ...
    def eval_model(self, device, model, eval_loader, save_folder, save_images, is_validation, plot_seg):
        ...
        seg_predictions, seg_ground_truths = [], []
        ...
        for data_point in eval_loader:
            ...
            prediction = self._get_inference_score(prediction, pred_seg)
            ...
            seg_predictions.append(pred_seg[0, 0, :, :])
            seg_ground_truths.append(seg_mask[0, 0, :, :])
        ...
        if is_validation:
            ...
            if self.cfg.TRAIN_MODE == "SEG_ONLY":
                ...
                self._log(
                    f"VALIDATION SEG || IoU={seg_metrics['mean_iou']:.5f}, Dice={seg_metrics['mean_dice']:.5f}, "
                    f"Precision={seg_metrics['mean_precision']:.5f}, Recall={seg_metrics['mean_recall']:.5f}"
                )
            ...
            if self.cfg.TRAIN_MODE == "SEG_ONLY":
                seg_metrics = utils.get_segmentation_metrics(np.array(seg_predictions), np.array(seg_ground_truths))
                utils.save_segmentation_metrics(seg_metrics, os.path.join(self.run_path, "segmentation_metrics.json"))
                self._log(
                    f"TEST SEG || IoU={seg_metrics['mean_iou']:.5f}, Dice={seg_metrics['mean_dice']:.5f}, "
                    f"Precision={seg_metrics['mean_precision']:.5f}, Recall={seg_metrics['mean_recall']:.5f}"
                )
    ...
    def _get_sample_score(self, decision, pred_seg):
        if self.cfg.TRAIN_MODE == "SEG_ONLY":
            return torch.max(pred_seg.reshape(pred_seg.size(0), -1), dim=1, keepdim=True)[0]
        return decision

    def _get_inference_score(self, decision, pred_seg):
        if self.cfg.TRAIN_MODE == "SEG_ONLY":
            seg_prob = torch.sigmoid(pred_seg)
            return torch.max(seg_prob.reshape(seg_prob.size(0), -1), dim=1, keepdim=True)[0]
