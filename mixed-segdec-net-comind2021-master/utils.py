...
def get_segmentation_metrics(predictions, labels, threshold=0.5):
    pred_bin = predictions >= threshold
    label_bin = labels >= 0.5

    intersection = np.logical_and(pred_bin, label_bin).sum(axis=(1, 2))
    union = np.logical_or(pred_bin, label_bin).sum(axis=(1, 2))
    pred_sum = pred_bin.sum(axis=(1, 2))
    label_sum = label_bin.sum(axis=(1, 2))

    iou = np.where(union > 0, intersection / (union + 1e-8), 1.0)
    dice = np.where((pred_sum + label_sum) > 0, (2.0 * intersection) / (pred_sum + label_sum + 1e-8), 1.0)
    precision = np.where(pred_sum > 0, intersection / (pred_sum + 1e-8), 1.0)
    recall = np.where(label_sum > 0, intersection / (label_sum + 1e-8), 1.0)
...
def save_segmentation_metrics(metrics, output_path):
